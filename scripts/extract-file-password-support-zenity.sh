...
: <<'DOCUMENTATION'
This Bash script is designed to facilitate the extraction and compression of various file types using a graphical user interface (GUI) provided 
by Zenity. It allows users to easily manage compressed files and folders with options for password protection and different compression formats.

Key Features:
1. File Extraction: Supports multiple formats including tar, zip, rar, and more, with options for password protection.
2. File Compression: Allows users to compress files and folders into various formats such as zip, 7z, and tar, with or without password protection.
3. User -Friendly Interface: Utilizes Zenity to provide a graphical interface for selecting files, entering passwords, and displaying output messages.
4. Error Handling: Includes checks for unsupported file types and invalid passwords, providing user feedback through GUI alerts.
5. Directory Navigation: Automatically navigates to the directory of the selected file for easier file management.

This script is a powerful tool for users needing to handle compressed files efficiently while providing a straightforward and interactive experience.
DOCUMENTATION
...
extract_file() {
        ...
        *.[Rr][Aa][Rr])
            if [[ -n "$password" ]]; then
                ...
                if [[ $? -eq 0 ]]; then
                    ...
                    zenity --warning --text="Invalid password for file: $file"
                    return 1
                fi
            else
    ...
    if [[ -n "$command" ]]; then
        ...
        if [[ $? -eq 0 ]]; then
            ...
            local warn='An error occurred during extraction!' && echo "$warn" >&2 && zenity --warning --text "$warn"
            return 1
        fi
    else
        echo 'Unsupported file extension!' >&2
...
compress_file() {
    ...
    case "$compression_type" in
        "zip")
            local option=$(zenity --list --title="Choose an option" --text="Select an option for zip compression" --radiolist --column="" --column="Option" TRUE "No password" FALSE "With password")
        ...
        "zst")
            ...
            ;;
        "7z")
            local option=$(zenity --list --title="Choose an option" --text="Select an option for 7z compression" --radiolist --column="" --column="Option" TRUE "No password" FALSE "With password")
            if [[ "$option" == "No password" ]]; then
            ...
            ;;
        "rar")
            local option=$(zenity --list --title="Choose an option" --text="Select an option for rar compression" --radiolist --column="" --column="Option" TRUE "No password" FALSE "With password")
            if [[ "$option" == "No password" ]]; then
    ...
    if [[ -n "$command" ]]; then
        ...
        if [[ $? -eq 0 ]]; then
            ...
            local warn='An error occurred during compression!' && echo "$warn" >&2 && zenity --warning --text "$warn"
            return 1
        fi
    fi
}
...
extract_dialog() {
    while true; do
        local option=$(zenity --list --title="Choose an option" --text="Option" --radiolist --column="" --column="Option" TRUE "Extract here" FALSE "Extract to a folder" FALSE "Create a compressed file")
