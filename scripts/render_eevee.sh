...
show_help() {
    echo "Usage: ${0##*/} -f <file> -o <directory> -t <type> [-r <resolution>] [-l <light_intensity>] [-s <samples>] [-a <camera_angle>]"
    echo "Options:"
    ...
    echo "  -l <light_intensity>    Light intensity (default: 8.0)"
    echo "  -s <samples>            Number of samples for rendering (default: 64)"
    echo "  -a <camera_angle>       Camera angle (e.g., top, bottom, front, back, left, right)"
    echo "  -h                      Display this help"
    echo "Example: ${0##*/} -f model.glb -o /tmp -t png -r 854x480 -l 10.0 -s 128 -a front"
    exit 1
}
...
while getopts "f:o:t:r:l:s:a:h" opt; do
    case $opt in
        f) INPUT_FILE="$OPTARG" ;;
        o) OUTPUT_DIR="$OPTARG" ;;
        t) FILE_TYPE=$(echo "$OPTARG" | tr '[:lower:]' '[:upper:]') ;;
        r) RESOLUTION="$OPTARG" ;;
        l) LIGHT_INTENSITY="$OPTARG" ;;
        s) SAMPLES="$OPTARG" ;;
...
if [ -n "$RESOLUTION" ]; then
    ...
    if ! [[ "$RES_X" =~ ^[0-9]+$ ]] || ! [[ "$RES_Y" =~ ^[0-9]+$ ]]; then
        echo "Error: Invalid resolution '$RESOLUTION'. Use the WxH format (e.g., 854x480)."
        exit 1
    fi
else
...
# Set render engine to EEVEE_NEXT
