//!DESC Cinema Color Grading (Tom Quente Moderado)
//!HOOK MAIN
//!BIND HOOKED

// Parâmetros ajustáveis
#define SATURATION_BOOST 1.35
#define CONTRAST 1.08
#define WARM_TINT 0.1
#define GOLDEN_HIGHLIGHTS 0.15
...
vec3 applyCinematicTone(vec3 color) {
    ...
    vec3 goldenHighlight = vec3(1.0, 0.92, 0.8);
    vec3 warmShadow = vec3(1.0, 0.9, 0.75);
    
    float highlightMask = pow(luminance, 2.0);
    float shadowMask = pow(1.0 - luminance, 2.0);
    
    // Aplica tons quentes de forma mais suave
    ...
    // Reforço sutil de tom quente geral
    color.r *= 1.04;
    color.g *= 1.02;
    color.b *= 0.98;
