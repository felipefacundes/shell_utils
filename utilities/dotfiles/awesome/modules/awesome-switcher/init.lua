...
function _M.getClients()
    local clients = {}
    local filtered_clients = get_non_panel_clients() -- Usa sua função global
    local s = mouse.screen
    local t = s.selected_tag
    
    -- Primeiro: adiciona clientes do histórico de foco (já filtrados)
    local idx = 0
    local c = awful.client.focus.history.get(s, idx)
    
    while c do
        -- Verifica se o cliente está na lista filtrada
        local is_filtered = false
        for _, fc in ipairs(filtered_clients) do
            if fc == c then
                is_filtered = true
                break
            end
    ...
    -- Segundo: adiciona clientes minimizados que não estão no histórico
    for i = 1, #filtered_clients do
        local c = filtered_clients[i]
        local ctags = c:tags()
        
        -- Verifica se o cliente está na tag atual
...
function _M.switch(dir, mod_key1, release_key, mod_key2, key_switch)
			...
			if gears.table.hasitem(mod, mod_key1) then
				...
				elseif key == key_switch and event == "press" then
					if gears.table.hasitem(mod, mod_key2) then
						...
						_M.cycle(-1)
						awful.client.focus.byidx(-1) -- fix bug of focus
					else
	...
	-- switch to next client
	_M.cycle(dir)
	awful.client.focus.byidx(dir) -- fix bug of focus
